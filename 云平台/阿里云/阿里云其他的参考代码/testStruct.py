...
def post_struct_data():
    ...
    # 构建结构体数据
    struct_data = {
        "test": test_value  # int32类型，范围0-100
    }
    ...
    payload = {
        ...
        "version": "1.0",
        "params": {
            "tempArry": struct_data  # 使用tempArry作为标识符
        }
    }
    ...
    if rc == 0:
        print(f"结构体数据上报成功, mid:{mid}")
        print(f"上报的结构体数据: {struct_data}")
    else:
        print(f"结构体数据上报失败:{rc}")

# 修改主循环，添加结构体上报选项
while True:
    try:
        msg = input("输入命令(1:断开连接 2:重连 3:发送消息 4:上报开启状态 5:上报关闭状态 6:上报结构体数据 q:退出)> ")
        if msg == "1":
            lk.disconnect()
        elif msg == "2":
            lk.connect_async()
